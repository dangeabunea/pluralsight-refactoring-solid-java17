...
public class TestHelpers {
    ...
    public static Optional<Method> getMethod(String className, String methodName){
        ...
        var method = Arrays.stream(classDefinition.getDeclaredMethods())
                .filter(f -> f.getName().equals(methodName))
                .findFirst();
        return method;
    }

    public static List<Constructor<?>> getConstructors(String className){
        final Optional<Class<?>> maybeClass = getClass(className);
        final Class<?> classDefinition = maybeClass.get();

        return Arrays.stream(classDefinition.getConstructors()).toList();
    }
}
